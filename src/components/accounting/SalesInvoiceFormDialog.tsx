...
export function SalesInvoiceFormDialog({ open, onOpenChange }: SalesInvoiceFormDialogProps) {
  const partners = useClinicStore((s) => s.partners);
  const accounts = useClinicStore((s) => s.accounts);
  const addSalesInvoice = useClinicStore((s) => s.addSalesInvoice);
  ...
  const [akunKasPiutangId, setAkunKasPiutangId] = useState("");
  ...
  const handleSubmit = () => {
    ...
    if (!akunKasPiutangId) {
      toast.error(lunasLangsung ? "Select the Kas/Bank account" : "Select the Piutang account");
      return;
    }
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>Records revenue and posts to the journal.</DialogDescription>
        </DialogHeader>
        ...
        <div className="grid max-h-[85vh] gap-3 overflow-y-auto overflow-x-hidden pr-1">
          ...
          <GoodsLinesEditor lines={lines} onChange={setLines} mode="sale" autofillPriceField="hargaJual" />
          ...
          <div className="grid gap-1.5">
            <Label>{lunasLangsung ? "Kas/Bank Account" : "Piutang Account"}</Label>
            <Select value={akunKasPiutangId} onValueChange={(v) => v && setAkunKasPiutangId(v)}>
              <SelectTrigger className="w-full">
